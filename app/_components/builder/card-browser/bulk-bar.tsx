...
/** Floating multi-select action bar; shown while items are selected. */
export function BulkBar({ target }: { target: string | null }) {
  const deck = useDeckBrowser();
  const n = deck.selected.size;
  const label = target ? toTitleCase(target) : "Mainboard";
  return (
    <div className="anim-slide-up flex items-center gap-3 rounded-xl border border-border bg-popover px-3.5 py-2.5 shadow-2xl">
