...
describe("toPrintingCreate", () => {
  ...
  describe("normalizeRarity", () => {
    it("capitalizes a lowercase rarity", () => {
      const p = toPrintingCreate(1, makeCard({ rarity: "rare" }));
      expect(p.rarity).toBe("Rare");
    });

    it("returns null for an unknown rarity", () => {
      const p = toPrintingCreate(1, makeCard({ rarity: "BOGUS" }));
      expect(p.rarity).toBeNull();
    });

    it("returns null when rarity is undefined", () => {
      const p = toPrintingCreate(1, makeCard({ rarity: undefined }));
      expect(p.rarity).toBeNull();
    });
  });
});
